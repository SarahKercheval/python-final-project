...
class first_window(tk.Toplevel):
    def __init__(self, root):
        ...
        b1.grid(column = 2, row = 1, padx = 80, pady = 20, ipadx = 5, ipady = 5)
        b2.grid(column = 2, row = 2, padx = 80, pady = 20)
        b3.grid(column = 2, row = 3, padx = 80, pady = 20)
...
class second_window(tk.Toplevel):
    def __init__(self):
        ...
        nextB = tk.Button(root, text="Next", command=lambda: self.recipe_specifics())
...
class recipe_editor():
    ...
    def recipe_editor_window(self, times, categories):
        ...
        window = tk.Frame(editor_window)
        window.rowconfigure(1, minsize = 800)
        window.columnconfigure(1, minsize = 800)

        #button to quit
        self.b_quit = tk.Button(text = 'Quit', command = lambda: quit_editor(self, editor_window))
        self.b_quit.grid(row = 1, column = 1)

        editor_window.mainloop()

        #method to quit and make sure that the user wants to quit before actually quitting
        def quit_editor(self, editor_window):
            quit_q = tk.Toplevel
            tk.Label(text = 'Are you sure you would like to quit?')
            self.b1 = tk.Button(text = 'Yes (Quit)', command = self.editor_window.destroy())
            self.b2 = tk.Button(text = 'No (Do not quit)', command = self.quit_q.destroy())
